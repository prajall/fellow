...
export const createExpenseSchema = z
  ...
  .refine(
    (data) => {
      const totalAllocated = data.participants.reduce((sum, p) => sum + p.allocated_amount, 0)
...
export type ExpenseParticipantFormData = z.infer<typeof expenseParticipantSchema>
